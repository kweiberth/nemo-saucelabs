...
var debug           = require('debug')('nemo-saucelabs');
    ...
    makeRequest: function makeRequest(message, callback) {
        ...
        request(message, function(err,response, body){
            ...
            if(response.statusCode !== 200) {
                throw new Error("Saucelabs reuqest is not success. Response status code is: " + response.statusCode);
            } else if(err){
                throw new Error("Error in processing Saucelabs request", err.message);
            }

            debug("Saucelabs Respsonse Status Code", response.statusCode);
